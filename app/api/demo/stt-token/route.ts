...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check kill switches
    if (env.DEMO_LIVE_PROVIDER_KILL_SWITCH === 'true' || env.CLOUDFLARE_AI_KILL_SWITCH === 'true') {
    ...
    if (apiKey) {
      const response = await fetch('https://api.elevenlabs.io/v1/single-use-tokens/scribe', {
        method: 'POST',
        headers: {
          'xi-api-key': apiKey,
