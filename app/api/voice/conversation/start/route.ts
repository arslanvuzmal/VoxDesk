...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const agentId = process.env.ELEVENLABS_AGENT_ID || process.env.ELEVENLABS_CONVAI_AGENT_ID;
    ...
    try {
      ...
      if (signedUrlRes.ok) {
        ...
        console.warn('[ELEVENLABS SIGNED URL ERROR]:', signedUrlRes.status, errText);
      }
    } catch (tokenErr) {
    ...
    console.error(`[CONVERSATION START ERROR] correlationId=${correlationId}:`, error);
    return NextResponse.json(
      {
