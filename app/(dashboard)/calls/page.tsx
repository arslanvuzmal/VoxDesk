  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400">Complete log of all inbound call sessions, transcripts, and outcomes.</p>
      </div>
          ...
          {calls.map((c) => (
            <div key={c.id} className="p-4 rounded-xl bg-gray-950 border border-gray-900 flex items-center justify-between text-xs">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-sm font-bold text-white">{c.caller} <span className="text-xs font-normal text-gray-400">({c.number})</span></h4>
                  <p className="text-gray-400 mt-0.5">{c.date} • Agent: <strong className="text-white">{c.agent}</strong> • Duration: {c.duration}</p>
                </div>
              </div>
