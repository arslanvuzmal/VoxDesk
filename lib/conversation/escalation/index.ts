...
export function detectEscalationTrigger(transcriptText: string): {
  ...
  if (
    ...
    return {
      shouldEscalate: true,
      reason: "Caller explicitly requested a human operator",
    };
  }
  if (
    ...
    text.includes("malpractice")
  ) {
    return {
      shouldEscalate: true,
      reason: "Urgent legal or emergency trigger phrase detected",
    };
  }
  if (
    ...
    return {
      shouldEscalate: true,
      reason: "Caller dissatisfaction / complaint trigger",
    };
  }
  return { shouldEscalate: false };
}

export function createTransferBrief(
  input: EscalationContextInput,
): HumanTransferBrief {
  ...
  if (
    input.triggerReason.includes("emergency") ||
    input.triggerReason.includes("urgent")
  ) {
    urgency = "critical";
  ...
  return {
    ...
    lastUnresolvedQuestion:
      input.lastUnresolvedQuestion || "General human assistance required",
