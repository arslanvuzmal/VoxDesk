...
export function searchProfileKnowledge(
  ...
  for (const trigger of profile.escalationTriggers) {
    ...
    const matchesTrigger = condWords.some(
      (w) => w.length > 3 && query.includes(w),
    );

    if (matchesTrigger) {
      const emergencyReply =
        profile.complianceDisclaimer[language] ||
        profile.complianceDisclaimer["en-US"];
  ...
  for (const restricted of profile.restrictedTopics) {
    ...
    const matchesRestricted =
      resWords.filter((w) => w.length > 4 && query.includes(w)).length >= 2;
  ...
  for (const item of profile.approvedKnowledge) {
    const matches = item.keywords.filter((kw) =>
