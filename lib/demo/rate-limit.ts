...
export async function validateSessionEligibility(
  ...
  if (env.DEMO_LIVE_PROVIDER_KILL_SWITCH === 'true') {
    return {
      eligible: false,
      reason: 'The live provider demonstration is temporarily paused for scheduled maintenance.',
  ...
  const cooldownSeconds = parseInt(env.DEMO_SESSION_COOLDOWN_SECONDS || '15', 10);
  const inCooldown = await demoSessionStore.checkIpCooldown(ipHash, cooldownSeconds);
  if (inCooldown) {
    return {
      eligible: false,
  ...
  const maxConcurrent = parseInt(env.DEMO_MAX_CONCURRENT_SESSIONS_GLOBAL || '10', 10);
  const activeCount = await demoSessionStore.getActiveSessionCount();
  if (activeCount >= maxConcurrent) {
    return {
      eligible: false,
      reason: 'All voice demonstration channels are currently busy. Please try again shortly.',
