...
export async function generateAgentTurn(
  ...
  if (!apiKey || apiKey.includes('your-') || apiKey.includes('placeholder')) {
    const fallbackData = getDeterministicFallback(userTranscript, scenario, profile, language);
    return {
      success: true,
      data: fallbackData,
      fallbackUsed: true,
  ...
  try {
    ...
    console.warn('[OPENROUTER LLM FALLBACK]:', error instanceof Error ? error.message : error);
  }
...
function parseStructuredOutput(raw: string, language: SupportedLanguage): VoiceAgentOutput | null {
  try {
    let str = raw;
...
export function getDeterministicFallback(
  ...
  if (knowledgeMatch.matched) {
    return {
      ...
      knowledgeReferences: knowledgeMatch.citation ? [knowledgeMatch.citation] : [],
  ...
  // Booking
    ...
    const sampleSlot = profile.appointmentSettings?.sampleSlots?.[0] || 'tomorrow at 2:00 PM PST';
  ...
  // Name provided
  else if (query.includes('my name is') || query.includes("i'm ") || query.includes('this is ')) {
