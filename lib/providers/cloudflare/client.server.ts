...
export async function validateCloudflareTokenServerSide(
  ...
  if (!isCloudflareAIEnabled()) {
    return {
      valid: false,
      reason: 'Cloudflare Workers AI is not configured or kill switch is enabled.',
    };
  }

  const now = Date.now();
  if (!forceFresh && cachedHealth && now - cachedHealth.checkedAt < CACHE_TTL_MS) {
  ...
  try {
    ...
    return {
      valid: false,
      reason: error instanceof Error ? error.message : 'Network error validating token',
    };
  }
}
