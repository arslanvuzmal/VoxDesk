...
async function provisionAgent() {
  ...
  const configuredAgentId = process.env.ELEVENLABS_AGENT_ID_LEGAL_EN?.trim() || process.env.ELEVENLABS_AGENT_ID?.trim();
  const voiceId = process.env.ELEVENLABS_VOICE_ID_LEGAL_EN?.trim() || process.env.ELEVENLABS_VOICE_ID?.trim() || "EXAVITQu4vr4xnSDxMaL";

  console.log("[PROVISIONING] Checking existing agent or provisioning ElevenLabs Northstar Legal Agent...");
